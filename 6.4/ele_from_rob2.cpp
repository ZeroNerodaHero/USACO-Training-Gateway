...
//if (s<5) cout << nx << " " << ny << " dist " << ret << endl;
            double delta = abs(d-ret);
            if(delta > 0.00001 && ret < d){
                d = ret;
                rx = nx;
                ry = ny;
     if (depth > maxdepth) { maxdepth = depth; if (maxdepth % 10000 == 0) cerr << "max: " << maxdepth << "   calls: " << ncalls << " , " << d << ' ' << ret << ' '<< rx << ' ' << ry << endl; } depth++;
                dfs(s);
cerr << "dfs returned!" << endl;
    depth--;
                return true;
            }
        }
